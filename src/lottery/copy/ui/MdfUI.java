...
public class MdfUI extends BaseDialog {
	...
	public void show(LotteryBlock m) {
		...
		cont.pane(p -> {
			...
			p.row();
			p.table(s -> {
				LUI.number(s, "底数 (level 1 + 2 + 3 + ... = 底数) : ", ip -> under = ip, () -> under, () -> !Vars.net.client(), 0, Integer.MAX_VALUE);
				s.row();
				for (int i = 0; i < pop.length; i++) {
					//s.add("level " + i + ": ");
					int finalI = i;
					LUI.number(s, "level " + i + ": ", ip -> pop[finalI] = ip, () -> pop[finalI], () -> !Vars.net.client(), 0, under);
				}
			}).left();
		});
